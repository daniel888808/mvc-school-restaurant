...
class show_all_costmer implements action_listener {

    public function actionPerformed($em) {
        $post = $em->getPost();
        $smember_type=$post['member_type'];
        ...
        switch ($member_type['member_type']['0']['member_type']) {
            case 1:
                $get_all_costmer = new user_profile_model();
                $all_costmer = $get_all_costmer->get_all_costmer('*',$smember_type);
                $return_value['status_code'] = 0;
                $return_value['status_message'] = 'Execute Success management';
                $return_value['member_type']=1;
                $return_value['data_set']=$all_costmer['data_set'];
                break;
            case 2:
                $return_value['status_code'] = 0;
